...
# Read the CSV files
events_df = pd.read_csv("/Users/salomonmuhirwa/Documents/NACB_PRoject/dataset/event1_data.csv")
...
# Aggregate the data
likes_agg = likes_df[likes_df['asset_type'] == 'event'].groupby('asset_id').size().reset_index(name='likes_count')
ratings_agg = ratings_df[ratings_df['asset_type'] == 'event'].groupby('asset_id').agg({'id': 'count', 'rate': 'mean'}).reset_index().rename(columns={'id': 'rating_count', 'rate': 'average_rating'})
saves_agg = saves_df[saves_df['asset_type'] == 'event'].groupby('asset_id').size().reset_index(name='saves_count')
views_agg = views_df[views_df['asset_type'] == 'event'].groupby('asset_id').size().reset_index(name='views_count')
...
aggregated_df.to_csv("/Users/salomonmuhirwa/Documents/NACB_PRoject/dataset/aggregated_event_data.csv", index=False)
